...
import org.bytedeco.javacpp.opencv_imgproc.{INTER_LINEAR, cornerSubPix}
import org.bytedeco.javacv.CanvasFrame

import scala.collection.mutable.ArrayBuffer

/**
  * Created by vassdoki on 2017.03.14..
  *
  * This is not used nor is working properly.
  *
  */
...
class CameraCalibrator {
  ...
  def addChessboardPoints(fileList: Seq[File], boardSize: Size): Int = {
    ...
    for (file <- fileList) {
      ...
      try {
        ...
        //Util.show(image, s"Calibration $file")
      }
    }
  ...
  private def convertPoints(): (MatVector, MatVector) = {
    ...
    for (((objectP, imageP), i) <- objectPoints zip imagePoints zipWithIndex) {
      objectPointsMatVect.put(i, Util.toMatPoint3f(objectP))
